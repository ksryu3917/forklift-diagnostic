...
public class TestPointLocatorActivity extends Activity {
    ...
    private String nextPointText(JSONObject r){StringBuilder sb=new StringBuilder();JSONArray a=r==null?null:r.optJSONArray("next_points");if(r!=null&&r.has("next_points")&&a!=null&&a.length()==0)return "";if(a!=null&&a.length()>0){for(int i=0;i<a.length();i++){String id=a.optString(i);JSONObject p=pointById(id);if(i>0)sb.append("\n");sb.append("[").append(id).append("] ").append(p==null?id:p.optString("label"));}}else{JSONArray pts=group.optJSONArray("points");if(pts!=null)for(int i=0;i<pts.length();i++){JSONObject p=pts.optJSONObject(i);String id=p.optString("id");if(!visiblePointIds.contains(id))continue;if(pref(id,"status").isEmpty()){sb.append("[").append(id).append("] ").append(p.optString("label"));break;}}}return sb.toString();}
