...
public class EngineSensorMapActivity extends Activity {
    ...
    private void render()throws Exception{
        ...
        Bitmap bm=drawIso();LinearLayout map=card();ImageView iv=new ImageView(this);iv.setImageBitmap(bm);iv.setAdjustViewBounds(true);iv.setScaleType(ImageView.ScaleType.FIT_CENTER);iv.setOnClickListener(v->ZoomImageDialog.show(this,bm));map.addView(iv,new LinearLayout.LayoutParams(-1,dp(360)));map.addView(tv("그림을 누르면 두 손가락 확대/드래그",11,false));body.addView(map);
    ...
    private void showAsset(String asset){try{InputStream is=getAssets().open(asset);Bitmap bm=BitmapFactory.decodeStream(is);is.close();if(bm==null)return;ZoomImageDialog.show(this,bm);}catch(Exception e){Toast.makeText(this,"부품도 이미지를 열 수 없습니다.",Toast.LENGTH_SHORT).show();}}
