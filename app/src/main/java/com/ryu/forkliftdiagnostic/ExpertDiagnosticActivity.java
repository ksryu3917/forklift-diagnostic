...
public class ExpertDiagnosticActivity extends Activity {
    ...
    private void render()throws Exception{
        ...
        if(!detail){
            Button more=btn("상세 근거 · 도면 · 분해조건 보기",false);more.setOnClickListener(v->{try{getIntent().putExtra("mode","detail");render();}catch(Exception e){fatal(e.toString());}});body.addView(more);
            return;
        }
        Button less=btn("← 빠른 화면으로",true);less.setOnClickListener(v->{try{getIntent().removeExtra("mode");render();}catch(Exception e){fatal(e.toString());}});body.addView(less);
